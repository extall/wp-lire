<?php
   ...
   * @author     Aleksei Tepljakov <alex@starspirals.net>
...
class Structures_BibTex
{
    ...
    public function parse()
    {
        ...
        // Expanding the Journal abbreviations
        if ($this->_options['fetchIEEEJournals'])
        {
            // Load the abbreviations
            $abbr = @file(dirname(__FILE__) . DIRECTORY_SEPARATOR . 'IEEEAbbr.txt');
            $allAbbrsRegex = array();
            $allJrnls = array();
            if (is_array($abbr))
            {
                foreach($abbr as $abbrLine)
                {
                    $abbrtemp = explode('=', $abbrLine);
                    if (count($abbrtemp) < 2) {
                        continue;
                    }
                    $allAbbrsRegex[] = '/#?' . trim($abbrtemp[0]) . '#?\b/';
                    $allJrnls[] = trim($abbrtemp[1]);
                }
            }

            // Expand journal abbreviations where needed
            if (!empty($allAbbrsRegex))
            {
                foreach($this->data as &$entry)
                {
                    if (array_key_exists('journal', $entry))
                    {
                        $entry['journal'] = preg_replace($allAbbrsRegex, $allJrnls, $entry['journal']);
                    }
                }
                unset($entry);
            }
    ...
    function _parseEntry($entry)
    {
        ...
        if ('@string' ==  strtolower(substr($entry, 0, 7))) {
            ...
            if ($this->_options['validate']) {
                if (!$this->checkAllowedEntryType($ret['entryType'])) {
     ...
     */
    function checkAllowedEntryType($entry)
    {
